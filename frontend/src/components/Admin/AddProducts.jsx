...
const AddProducts = () => {
  ...
  const initForm = { title: '', imglink: '', price: '', mrp: '',rating:'', category: '' }
      ...
      <Flex>
        <Box mr={'5'}>
          <form onSubmit={formSubmitHandler}>
            <FormControl isRequired>
              ...
              <Input type='text' name='title' background='#fff' htmlSize={45} width='auto' onChange={formChangeHandler} value={form.title} />
              <FormLabel>Product Image Link</FormLabel>
              <Input type='url' name='imglink' background='#fff' onChange={formChangeHandler} value={form.imglink} />
              ...
              <Input type='text' name='mrp' background='#fff' onChange={formChangeHandler} value={form.mrp} />
        ...
        <Flex flexDir={'column'} h={'60vh'} alignItems={'center'} justifyContent='center' textAlign='center' bg='white' w={'full'} maxW={'300px'} role={'group'} boxShadow={'2xl'} rounded={'lg'} pos={'relative'} zIndex={1}>
          <Flex rounded={'lg'} justifyContent={'center'} alignItems={'center'}  width={'16vw'} textAlign={'center'} pos={'relative'} _after={{transition: 'all .3s ease', content: '""', w: 'full', h: 'full', pos: 'absolute', top: 5,   left: 0,   filter: 'blur(15px)',   zIndex: -1,
           ...
           {form.imglink && <Image rounded={'lg'} boxSize={250} objectFit={'cover'} src={form.imglink} />}
          </Flex>
          <Stack align={'center'}>
            {form.title && <Heading fontSize={'md'} fontFamily={'body'} fontWeight={500}>{form.title}</Heading>}
            {form.price && <Heading fontSize={'md'} fontFamily={'body'} color={'blue.800'} fontWeight={400}>{form.price}</Heading>}
            {form.category && <Heading fontSize={'md'} fontFamily={'body'} color={'blue.800'} fontWeight={400}>{form.category}</Heading>}
          </Stack>
        </Flex>
      </Flex>
    </div>
